...
public class BTPiece implements BTMessage {
  ...
  /* 
   * */
  private DirectByteBuffer tempbuff = null;
  
  private int keyreq;
  private byte[] keyreqfrom = new byte[20];
  private byte[] nexthop = new byte[20];
   ...
   *
   * */
  public BTPiece( int piece_number, int piece_offset, int keyreq, byte[] keyreqfrom, byte[] nexthop, DirectByteBuffer data, byte version ) {
    this.piece_number = piece_number;
    this.piece_offset = piece_offset;
    this.piece_length = data == null ? 0 : data.remaining( DirectByteBuffer.SS_MSG );
    
    //tempbuff =data;
    tempbuff = DirectByteBufferPool.getBuffer(DirectByteBuffer.AL_MSG_BT_PAYLOAD, piece_length);
    tempbuff.put(DirectByteBuffer.AL_MSG_BT_PIECE, data);
    tempbuff.flip(DirectByteBuffer.AL_MSG_BT_PIECE);
    //data.position(DirectByteBuffer.AL_MSG_BT_PIECE, 0);
    this.keyreq = keyreq;
    this.keyreqfrom = keyreqfrom;
    this.nexthop = nexthop;
    
    buffer[1] = DirectByteBufferPool.getBuffer( DirectByteBuffer.AL_MSG_BT_PIECE, piece_length + 44 );
    buffer[1].putInt(DirectByteBuffer.SS_MSG, keyreq);
    buffer[1].put(DirectByteBuffer.SS_MSG, keyreqfrom);
    buffer[1].put(DirectByteBuffer.SS_MSG, nexthop);
    buffer[1].put(DirectByteBuffer.SS_MSG, tempbuff);
    buffer[1].flip( DirectByteBuffer.SS_MSG );
    
    tempbuff.position(DirectByteBuffer.SS_MSG, 0);
    Debug.out("Whatever: " + tempbuff.position(DirectByteBuffer.AL_MSG_BT_PIECE) + "Whatever: " + tempbuff.limit(DirectByteBuffer.AL_MSG_BT_PIECE) + "Whatever: " + tempbuff.capacity(DirectByteBuffer.AL_MSG_BT_PIECE));
...
public int getPieceNumber() {  return piece_number;  }
  ...
  public DirectByteBuffer getPieceData() {  /*return buffer[1];*/   return tempbuff ; }
  ...
  public Message deserialize( DirectByteBuffer data, byte version ) throws MessageException {    
    ...
    int keyreq = data.getInt(DirectByteBuffer.SS_MSG);
    byte[] keyreqfrom = new byte[20];
    data.get( DirectByteBuffer.SS_MSG, keyreqfrom ); 
    Debug.out("DS: Keyreqfrom =" + keyreq);
    byte[] nxthop = new byte[20];
    data.get( DirectByteBuffer.SS_MSG, nxthop ); 
    Debug.out("DS: fromnxthopkeyreqfrom =" + PeerClassifier.getPrintablePeerID(keyreqfrom));
    return new BTPiece( number, offset, keyreq, keyreqfrom, nxthop, data, version);
