...
implements PEPeerTransport
{
	...
	//110
	private T1Module t1mod = new T1Module();
	private T2Module t2mod = new T2Module();
	private T3Module t3mod = new T3Module();
	...
	protected void decodeBTHandshake( BTHandshake handshake ) {
		...
		{	
			/*
			 * byte[] receiptpieceno = handshake.getDataHash();
			int rptpn = ByteBuffer.wrap(receiptpieceno).getInt();
			if (rptpn == -5){
				Debug.out("Got receipt handshake");
			}
			byte[] receipttermination = handshake.getPeerId();
			if ((rptpn>=0)) {
	        	Debug.out("Debug: got receipt for " + PeerClassifier.getPrintablePeerID( receipttermination) + receipttermination.length);
				PEPeerTransport keypt = manager.getTransportFromIdentity(receipttermination);
				if (keypt != null){
					Debug.out("decode TCHAIN: Received receipt sending T-KEY");
					keypt.sendT4Key(rptpn, peer_id);
					//piece.destroy();
					handshake.destroy();
					return;
				}
  ...
  protected void decodeAZHandshake(AZHandshake handshake) {
	...
	public void sendT4Key(int keyreq, byte[] destination){
		final byte[] nothing = new byte[] { (byte)0x88, (byte)0x88, (byte)0x88, 
            	(byte)0x88, (byte)0x88, (byte)0x88, (byte)0x88, (byte)0x88, 
            	(byte)0x88, (byte)0x88, (byte)0x88, (byte)0x88, (byte)0x88,
            	(byte)0x88, (byte)0x88, (byte)0x88, (byte)0x88, (byte)0x88, 
            	(byte)0x88, (byte)0x88};
		final byte[] extnothing = new byte[] { (byte)0x11, (byte)0x22, (byte)0x33, 
            	(byte)0x44, (byte)0x55, (byte)0x66, (byte)0x77, (byte)0x88, 
            	(byte)0x88, (byte)0x88, (byte)0x88, (byte)0x88, (byte)0x88,
            	(byte)0x88, (byte)0x88, (byte)0x88, (byte)0x88, (byte)0x88, 
            	(byte)0x88, (byte)0x88};
		if (this.t1mod.checkPiecekey(keyreq)){
			if (this.t1mod.getFromTranslator(keyreq).size()>0){
				if (this.t1mod.getFromTranslator(keyreq).contains(destination)){
					byte[] secret = this.t1mod.getFromPiecekey(keyreq);
					
					DirectByteBuffer secretkey = new DirectByteBuffer(ByteBuffer.wrap(secret));
					DirectByteBuffer seckey = DirectByteBufferPool.getBuffer(DirectByteBuffer.AL_MSG_BT_PIECE, secret.length);
				    seckey.put(DirectByteBuffer.SS_MSG, secret);
				    seckey.flip(DirectByteBuffer.SS_MSG);
					
				    BTPiece keykey = new BTPiece(0, 0, keyreq, extnothing , extnothing, seckey, (byte)1);
					
					this.connection.getOutgoingMessageQueue().addMessage( keykey, false );
					Debug.out("Sent tchain key to" + this.getIp());
					this.t1mod.removeFromTranslator(keyreq, destination);
					if (this.t1mod.getFromTranslator(keyreq) == null){
						this.t1mod.removeFromPiecekey(keyreq);
					}
					return;
				}
			}
	...
	public void sendT3Receipt(int keyreq, byte[] destination){
        final byte[] receipt = new byte[] { (byte)0xaa, (byte)0xaa, (byte)0xaa,
            	(byte)0xaa, (byte)0xaa, (byte)0xaa, (byte)0xaa, (byte)0xaa,
            	(byte)0xaa, (byte)0xaa, (byte)0xaa, (byte)0xaa, (byte)0xaa,
            	(byte)0xaa, (byte)0xaa, (byte)0xaa, (byte)0xaa, (byte)0xaa,
            	(byte)0xaa, (byte)0xaa };
        byte[] rptnxthop = receipt;
        final DirectByteBuffer rptdata = DirectByteBufferPool.getBuffer( 
        		DirectByteBuffer.AL_MSG_BT_PIECE, "This is just a T-Chain Receipt!".getBytes().length);
        rptdata.put(DirectByteBuffer.SS_MSG, "This is just a T-Chain Receipt!".getBytes());
        rptdata.position(DirectByteBuffer.SS_MSG, 0);
		
        byte[] rptdatahash = ByteBuffer.allocate(20).putInt(keyreq).array();
        Debug.out(" Receipt for peer " + destination.toString());
        BTHandshake rpths = new BTHandshake(rptdatahash, destination, false, (byte)1);        
        
        BTPiece rpt = new BTPiece(0, 0, keyreq, destination, rptnxthop, rptdata, (byte)1);
        if (this.isChokedByMe()){
        	Debug.out("Final terminator is isChokedByMe");
        }
        else{
        	Debug.out("Not isChokedByMe");
        }
        //if(this.isChokedByMe()){
        //	this.sendUnChoke();
        //	Debug.out("Unchoking final tchain since choked");
        //	
        //}
        connection.getOutgoingMessageQueue().addMessage( rpt, false );
    	
        Debug.out("Sent the receipt to " + this.getIp());
        return;
		//connection.getOutgoingMessageQueue().addMessage( rpt, false );
		
	}
	
	public void doT2continuation(int piecenumb, byte[] initiator){
		this.t2mod.putToContinuation(piecenumb, initiator);
		//do peer unchoke
		if (isChokedByMe() && isInterested()){
			sendUnChoke();
		}	
		//sendHave(piecenumb);
		return;
	...
	public ArrayList<byte[]> doT2Reciprocation(){
		return (this.t2mod.Reciprocation());
	...
	protected void decodePiece( BTPiece piece ) {
		...
		//
		final int keyreq = piece.getKeyreq();
		final byte[] keyreqfrom = piece.getKeyreqfrom();
		final byte[] nexthop = piece.getNexthop();
		...
		//String keyreqfromstring = "";
		//try {
		//	keyreqfromstring = new String(keyreqfrom, "UMF-8");
		//} catch (UnsupportedEncodingException e) {
		//	// TODO Auto-generated catch block
		//	e.printStackTrace();
		//}
        //Debug.out("Debug: got receipt re" + keyreq);
        //Debug.out("Debugdecode: pieceno" + pieceNumber);
        //Debug.out("Debugdecode: offset" + offset);
        //Debug.out("Debug: got receipt re" + keyreq);
        
        if ((keyreq>=0)) {
        	Debug.out("Debug: got receipt");
			if ((pieceNumber == 0) && (offset == 0) && (Arrays.equals(nexthop, receipt))){
				Debug.out("Debug: got receipt re");
				PEPeerTransport keypt = manager.getTransportFromIdentity(keyreqfrom);
				if (keypt != null){
					Debug.out("decode TCHAIN: Received receipt sending T-KEY");
					keypt.sendT4Key(keyreq, peer_id);
					piece.destroy();
					return;
				}
			}
        ...
        if (pieceNumber >= 0 && offset >= 0 && Arrays.equals(nexthop, nothing) && (!Arrays.equals(keyreqfrom, extnothing))){
        	
        	Debug.out("decoding without tchain");
			
        	//
			if ((keyreq >= 0) && (!Arrays.equals(keyreqfrom, nothing))){
				
				Debug.out("decode TCHAIN: Peer asking for some other receipt");
				
				PEPeerTransport krfpt = manager.getTransportFromIdentity(keyreqfrom);
				if (krfpt !=null){

					krfpt.sendT3Receipt(keyreq, peer_id);
					Debug.out("decode TCHAIN: Sent receipt");
        	...
        	final Object error_msg = 
        	  ...
        	  if ( DEBUG_FAST ){
        		...
        		// checking if has been requested in prev requests
		...
		if ((pieceNumber == 0) && (offset == 0) && (Arrays.equals(keyreqfrom, extnothing))){ 
				//&& (Arrays.equals(keyreqfrom, nothing)) && (Arrays.equals(nexthop, nothing))){
			Debug.out("Keyreqfrom " + keyreqfrom.toString() + "and extnothing" + extnothing.toString());
			int newoffset = this.t3mod.getFromPieceblock(keyreq).get(0);
			byte[] realda = this.t3mod.getFromPiecedata(keyreq).get(0);
			
			
			//DirectByteBuffer newpayload2 = DirectByteBufferPool.getBuffer(
			//		DirectByteBuffer.SS_MSG, this.t3mod.getFromPiecedata(keyreq).get(0).length);
			// DirectByteBuffer newpayload2 = new DirectByteBuffer (ByteBuffer.wrap(this.t3mod.getFromPiecedata(keyreq).get(0)));
			int newpieceNumber = keyreq;
			
			byte[] newkey = new byte[payload.remaining(DirectByteBuffer.SS_MSG)];
			payload.get(DirectByteBuffer.SS_MSG, newkey);
			payload.position(DirectByteBuffer.SS_MSG, 0);
			DirectByteBuffer newpayload = DirectByteBufferPool.getBuffer(
					DirectByteBuffer.SS_MSG, realda.length);
			
			Debug.out("decode TCHAIN: Final KEY received. Sent for decrypt " + realda.length + " : " + newkey.length);
			
			Debug.out("New key got is " + Arrays.toString(newkey) + "new offset is" + newoffset);
			Debug.out("New key payload buffer = " + payload.remaining(DirectByteBuffer.SS_MSG));
			//Cipher cipher;
			try {
				Cipher cipher = Cipher.getInstance("AES/ECB/NoPadding");
				SecretKey key2 = new SecretKeySpec(newkey, 0, newkey.length, "AES");
				
				
				cipher.init(Cipher.DECRYPT_MODE, key2);
				byte[] realpayld = cipher.doFinal(realda);
				newpayload = new DirectByteBuffer (ByteBuffer.wrap(realpayld));
			...
			this.t3mod.removeFromPieceblock(keyreq);
			this.t3mod.removeFromPiecedata(keyreq);
			
			int newlength = newpayload.remaining( DirectByteBuffer.SS_PEER );
			
			Debug.out("Received T-Chain Piece!! " + newpieceNumber + ": " + newoffset);
			...
			final Object error_msg = 
			  ...
			  if( !manager.validatePieceReply( this, newpieceNumber, newoffset, newpayload ) ) {
					peer_stats.bytesDiscarded( newlength );
					manager.discarded( this, newlength );
					requests_discarded++;
					printRequestStats();
					Debug.out(" piece discarded as invalid");
					piece.destroy();
					if (Logger.isEnabled())
						Logger.log(new LogEvent(this, LOGID, LogEvent.LT_ERROR,
								error_msg
			  ...
			  if ( DEBUG_FAST ){
				  if ( fast_extension_enabled && really_choked_by_other_peer ){
					  ...
					  System.out.println( "Received allow-fast piece for " + newpieceNumber + "/" + newoffset + "/" + length + " from " + getIp());
				  }
			  }
				final DiskManagerReadRequest request = manager.createDiskManagerRequest( newpieceNumber, newoffset, newlength );
				boolean piece_error = true;
				// checking if has been requested in prev requests
				if( hasBeenRequested( request ) ) { 
					Debug.out("From active request");//from active request
					removeRequest( request );
					final long now =SystemTime.getCurrentTime();
					resetRequestsTime(now);

					if( manager.isWritten( newpieceNumber, newoffset ) ) {  //oops, looks like this block has already been written
						peer_stats.bytesDiscarded( newlength );
						Debug.out("Oops already written");
						manager.discarded( this, newlength );
						Debug.out(" piece ignored as already written");
						if( manager.isInEndGameMode() ) {  //we're probably in end-game mode then
							if (last_good_data_time !=-1 &&now -last_good_data_time <=60 *1000)
								setSnubbed(false);
							last_good_data_time =now;
						...
						manager.writeBlock( newpieceNumber, newoffset, newpayload, this, false);
						Debug.out("Successfully written the tchain block");
						if (last_good_data_time !=-1 &&now -last_good_data_time <=60 *1000)
							setSnubbed( false );
						last_good_data_time =now;
						requests_completed++;
					...
					if( !manager.isWritten( newpieceNumber, newoffset ) ) {
						final boolean ever_requested;

						try{  recent_outgoing_requests_mon.enter();
						Debug.out("Expired request");
						ever_requested = recent_outgoing_requests.containsKey( request );
						}
						finally{  recent_outgoing_requests_mon.exit();  }

						if( ever_requested ) { //security-measure: we dont want to be accepting any ol' random block
							manager.writeBlock( newpieceNumber, newoffset, newpayload, this, true);
							final long now =SystemTime.getCurrentTime();
							if (last_good_data_time !=-1 &&now -last_good_data_time <=60 *1000)
								setSnubbed(false);
							resetRequestsTime(now);
		...
		if (pieceNumber >= 0 && offset >= 0 && (!Arrays.equals(nexthop, nothing)) && (!Arrays.equals(keyreqfrom, extnothing))){
			Debug.out("decode TCHAIN: Found piece with Tchain properties. Saving it..");
			if (nexthop.length == 20 && !(Arrays.equals(nexthop, receipt)) && !(Arrays.equals(nexthop, nothing))){
				PEPeerTransport pt = manager.getTransportFromIdentity(nexthop);
				
				if (pt != null){
					Debug.out("decode TCHAIN: Found final peer");
					
					this.t3mod.putToPieceblock(pieceNumber, offset);
					byte[] payloadba = new byte[payload.remaining(DirectByteBuffer.SS_MSG)];
				    payload.get( DirectByteBuffer.SS_MSG, payloadba );
				    payload.position(DirectByteBuffer.SS_MSG, 0);
					this.t3mod.putToPiecedata(pieceNumber, payloadba);
					
					Debug.out("decode TCHAIN: Added data for later decrypt" + payloadba.length);
					
					pt.doT2continuation(pieceNumber, peer_id);
					
					Debug.out("decode TCHAIN: Done T2 continuation in another peer");
    ...
    if ( AEDiagnostics.CHECK_DUMMY_FILE_DATA ){
      ...
      payload.position( DirectByteBuffer.SS_PEER, pos );
		 ...
		 */
		Debug.out("--Keyreq: " + piece.getKeyreq());
		Debug.out("--Keyreqfrom: " + PeerClassifier.getPrintablePeerID(piece.getKeyreqfrom()));
		Debug.out("--nexthop: " + PeerClassifier.getPrintablePeerID(piece.getNexthop()));
		// 3913
		/*
		final Object error_msg = 
			new Object()
	...
	*/
	...
	public int 
 	doT1Piecekeyjob(int pieceno,byte[] keygenerated){
		this.t1mod.putToPiecekey(pieceno, keygenerated);
		Debug.out("Inside Peer TCHAIN: putting piece key to mod1");
		return 0;
		
	}
	public boolean 
	checkT1Piecekey(int pieceno){
		return this.t1mod.checkPiecekey(pieceno);
	
	}
	public byte[] 
	getT1Piecekey(int pieceno){
		return this.t1mod.getFromPiecekey(pieceno);
	}
	public int 
 	doT1Translatorjob(int pieceno,byte[] nexthop){
		this.t1mod.putToTranslator(pieceno, nexthop);
		Debug.out("Inside Peer TCHAIN: Adding nexthop pieceno to T1mod");
		return 0;
		
	}
	public ArrayList<PEPeer>
	doT1RandomNexthop(){
		final List<PEPeer> allpeer = this.manager.getPeers();
		ArrayList<PEPeerTransport> knownpeer = new ArrayList<PEPeerTransport>();
		PEPeerTransport thispeer = null;
		for ( int i =0; i<allpeer.size(); i++){
			final PEPeerTransport peer =(PEPeerTransport) allpeer.get(i);
			byte [] mahatemp = peer.getId();
			
			if ( manager.getTransportFromIdentity(mahatemp) == this ){
				thispeer = peer;
				continue;
			}
			if (manager.getTransportFromIdentity(mahatemp) != this && 
					(!manager.getTransportFromIdentity(mahatemp).isSeed() && 
							!manager.getTransportFromIdentity(mahatemp).isRelativeSeed())){
				if (peer.getConnectionState() == PEPeerTransport.CONNECTION_FULLY_ESTABLISHED){
					knownpeer.add(peer);
					Debug.out("Added p = peer" + peer.getClient());
		...
		for (int j = 0; j<knownpeer.size();j++){
			BitFlags bitmap = knownpeer.get(j).getAvailable();
			if (bitmap == null){
				knownpeer.remove(j);
				continue;
			}
			if (knownpeer.get(j).getAvailable().nbSet >= thispeer.getAvailable().nbSet){
				knownpeer.remove(j);
			}
		}
		if (knownpeer.size()==0){
		...
		ArrayList<PEPeer> pe = new ArrayList();
        
 
        	
    	Debug.out("Inside Peer TCHAIN: Sending random next hop");
    	int randomindex = randomGenerator.nextInt(knownpeer.size());
    	pe.add(knownpeer.get(randomindex));
