		...
		for (int i =peer_transports.size() -1; i >=0 ; i--)
		{
			...
			if (pc.getPeerState() ==PEPeer.TRANSFERING)
			{
				...
				{   // now we know there's a request that's > 60 seconds old
					...
					if (timeSinceGoodData <0 ||timeSinceGoodData >600 *1000)
						pc.setSnubbed(true);
					...
					for (int j = (timeSinceOldestRequest >1200 *1000 && noData)  ? 0 : 1; j < expired.size(); j++)
					{
							...
							for (int i =0; i <pePiece.getNbBlocks(); i++ ){
								...
								if (correctHash !=null)
								{
									...
									while (iterPerBlock.hasNext())
									{
										...
										{
											//Bad peer found here
											Debug.out("Bad peer hash recheck with correctHash failed ");
